...
namespace ServerApp
{
    public partial class MainWindow : Window
    {
        ...
        private bool isRunning; private CancellationTokenSource cts;
        private ObservableCollection connectedUsers;

        public MainWindow()
        {
            InitializeComponent();
            connectedUsers = new ObservableCollection<ClientInfo>();
            UsersListBox.ItemsSource = connectedUsers;
            isRunning = false;
            Log("Сервер готов к запуску.");
        }
        ...
        private void StopButton_Click(object sender, RoutedEventArgs e)
        {
            if (isRunning)
            {
                ...
                clients.Clear();
                Dispatcher.Invoke(() => connectedUsers.Clear());
        ...
        private async void StartServerAsync()
        {
            try
            {
                ...
                listener.Start();
                Log($"Сервер запущен на IP-адресе {myIp} и порту {port}");

                while (!cts.Token.IsCancellationRequested)
                {
                    TcpClient client = await listener.AcceptTcpClientAsync();
                    ThreadPool.QueueUserWorkItem(ClientHandler, new ClientState { TcpClient = client, CancellationToken = cts.Token });
                }
            }
            catch (Exception ex)
            {
        ...
        private void InitializeDatabase()
        {
            ...
            cmd.ExecuteNonQuery();
            cmd.CommandText = createGroupChatsTableSql;
            cmd.ExecuteNonQuery();
            cmd.CommandText = createGroupMembersTableSql;
            cmd.ExecuteNonQuery();
            cmd.CommandText = createGroupMessagesTableSql;
            cmd.ExecuteNonQuery();
        }
        ...
        private void ClientHandler(object state)
        {
            ...
            TcpClient tcpClient = clientState.TcpClient;
            CancellationToken token = clientState.CancellationToken;
            ...
            try
            {
                ...
                else if (request.Type == "login")
                {
                    ...
                    var clientInfo = new ClientInfo { Name = userName, Client = tcpClient, Stream = stream };
                    if (!clients.TryAdd(userName, clientInfo))
                    {
                        SendMessage(stream, new Message { Type = "login_fail", Text = "Пользователь уже подключен." });
                        tcpClient.Close();
                        return;
                    }
                    Dispatcher.Invoke(() => connectedUsers.Add(clientInfo));
                    ...
                    while (!token.IsCancellationRequested)
                    {
                        ...
                        {
                            Log($"Пользователь {userName} отключается по запросу клиента.");
                            break;
                        }
                        else if (message.Type == "invite_to_group")
                        {
                            ...
                            cmd.Parameters.AddWithValue("@user", message.Text);
                            cmd.ExecuteNonQuery();
            ...
            finally
            {
                if (userName != null)
                {
                    ...
                    Dispatcher.Invoke(() => connectedUsers.Remove(clients.Values.FirstOrDefault(c => c.Name == userName)));
                    BroadcastUserList();
        ...
        private bool AddUser(string login, string username, string password)
        {
            ...
            {
                Log($"Ошибка добавления пользователя: {ex.Message}");
                return false;
            }
        }

        private void UpdateUserPassword(string login, string newHash)
        {
            try
            {
                using var connection = new SQLiteConnection(ConnectionString);
            ...
            {
                Log($"Ошибка обновления пароля: {ex.Message}");
            }
        }

        private void SaveDialogMessage(string dialogKey, string sender, string message)
        {
            try
            {
                using var connection = new SQLiteConnection(ConnectionString);
            ...
            {
                Log($"Ошибка сохранения сообщения чата: {ex.Message}");
            }
        }

        private List<string> LoadDialogMessages(string dialogKey)
        {
            try
            {
                using var connection = new SQLiteConnection(ConnectionString);
            ...
            {
                Log($"Ошибка загрузки сообщений чата: {ex.Message}");
        ...
        private List<string> GetAllUsersFromDatabase()
        {
            ...
            {
                Log($"Ошибка при получении списка пользователей из БД: {ex.Message}");
            }
            return users;
        }
        ...
        private void SaveGroupMessage(string groupName, string sender, string message)
        {
            using var connection = new SQLiteConnection(ConnectionString);
            connection.Open();
        ...
        private List<string> LoadUserGroups(string user)
        {
        ...
        private List<string> LoadGroupMessages(string groupName)
        {
            ...
            while (reader.Read())
            {
                ...
                messages.Add($"{sentAtTargetTimeZone:yyyy.MM.dd HH:mm:ss} {sender}: {text}");
            }
            return messages;
        }

        private void BroadcastGroupMessage(string groupName, string sender, string text)
        {
            using var connection = new SQLiteConnection(ConnectionString);
            connection.Open();
            long groupId = GetGroupId(groupName, connection);
        ...
        private long GetGroupId(string groupName, SQLiteConnection connection)
        {
